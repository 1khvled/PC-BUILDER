  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-xs space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-blue-50 text-[#0b63e5] border border-blue-100">
        ...
        <div className="pt-2 flex flex-wrap items-center justify-between gap-4 bg-slate-950 text-white rounded-2xl p-5 sm:p-6 mt-4 shadow-lg">
          <div>
            <div className="text-xs text-slate-400 font-semibold uppercase tracking-wider">
              Budget total estimé (prix les plus bas relevés) :
            </div>
            <div className="text-2xl sm:text-3xl font-black text-emerald-400 font-mono tracking-tight mt-1">
          ...
          <Link
            href="/builder"
            className="px-5 py-3 rounded-xl bg-[#0b63e5] hover:bg-[#094db5] text-white font-bold text-xs sm:text-sm shadow-md transition-all card-lift flex items-center gap-2 shrink-0"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[620px]">
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {rows.map(({ p, best }) => (
                <tr key={p!.id} className="rowline hover:bg-blue-50/30 transition-colors group">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-xs space-y-8">
          ...
          <section key={i} className="space-y-3">
            {b.h && (
              <h2 className="font-black text-xl text-slate-900 tracking-tight flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-[#0b63e5]" />
                <span>{b.h}</span>
              </h2>
            )}
            {(b.p ?? []).map((t, j) => (
        ...
        {/* Pitfalls Callout */}
        <section className="bg-amber-50/80 border border-amber-200/90 rounded-2xl p-5 sm:p-6 space-y-3">
