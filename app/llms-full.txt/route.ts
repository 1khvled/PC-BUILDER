import { OFFERS, PRODUCTS, bestOffer } from "@/lib/data/products";
...
export async function GET() {
  const lines: string[] = [
    ...
    `Relevé: ${SCRAPED_AT.slice(0, 10)}. Monnaie: DZD (DA). Tri organique par prix croissant.`,
  ...
  for (const p of PRODUCTS) {
    ...
    const offers = OFFERS.filter((o) => o.productId === p.id).sort((a, b) => a.priceDa - b.priceDa);
    if (!offers.length) lines.push("Aucune offre indexée.");
  ...
  for (const g of GUIDES) {
    const total = g.parts.reduce((s, id) => s + (bestOffer(id)?.priceDa ?? 0), 0);
