...
export default function BuildsPage() {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="bg-white rounded border border-slate-200 p-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="relative flex-1 max-w-xs min-w-[220px]">
          <input
            ...
            className="w-full border border-slate-200 rounded pl-8 pr-7 py-1.5 text-xs bg-slate-50 focus:bg-white focus:border-[#2c87c3] outline-none transition-colors"
          />
          <svg
              ...
              >
                <div>
                  ...
                  <div className="flex items-start gap-4">
                    <div className="p-2 rounded bg-slate-50 border border-slate-200/80 shrink-0 shadow-inner">
                      <Thumb src={gpu ? productImage(gpu) : undefined} alt={b.title} size={68} />
                    </div>

                    <div className="min-w-0 flex-1">
                      <h2 className="font-black text-lg text-slate-900 group-hover:text-[#2c87c3] transition-colors leading-snug">
                        {b.title}
                      </h2>

                      {/* Budget Badge */}
                      <div className="text-xl font-black text-emerald-700 tracking-tight mt-1">
                ...
                <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap gap-1.5 text-[11px]">
                    ...
                    {ram && (
                      <span className="px-2.5 py-1 rounded bg-slate-100 text-slate-700 font-medium hidden sm:inline">
                        RAM : <b className="text-slate-900">{ram.model}</b>
                      </span>
                    )}
                  </div>

                  <span className="text-xs font-bold text-[#2c87c3] flex items-center gap-1">
